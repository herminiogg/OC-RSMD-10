...
11. In order to avoid the two-step CodeMeta generation process and avoid future problem when Zenodo implementens the support for CodeMeta, the two conflicting attributes (namely, the publicationDate and the releaseNotes) which are now pulled from the Github releases API path. In addition, the attributes issueTracker and downloadUrl were not process in the JSON-LD framed version due to a problem in the IRI format. Apart from that, the role of the author was added and a referencePublication too.
